...
def _strip_html(s: Optional[str]) -> Optional[str]:
    if not isinstance(s, str):
        return s
    # decode HTML entities and strip tags like <em>H</em>
    s2 = unescape(s)
...
# MF / MW helpers (robust against schema differences). If not found, compute from SMILES via RDKit.
...
class CASSettings(BaseSettings):
    server: str = "needs to be set"
    token_url: str = "needs to be set"
    grant_type: str = "client_credentials"
    client_id: str = "needs to be set"
    client_secret: str = "needs to be set"
    scope: str = "cas_content"
    cache_file: str = ".cas_token_cache.json"

    verify_ssl: bool = True
    debug: bool = False
    details_on_lookup: bool = True

    # NEW: rate limit (calls/second). 0 disables limiting.
...
class RequestHandler:
    ...
    def post(self, url: str, json_body: Optional[dict] = None) -> dict:
        ...
        self._limiter.wait()  # enforce rate BEFORE sending
...
class CASClient:
    """
    Public API:
      - lookup_by_smiles(smiles, full=False, wide=False, mf_mw=False)
      - lookup_by_cas(rn, full=False, wide=False, mf_mw=False)
      - lookup_by_name(name, full=False, wide=False, mf_mw=False)

    Endpoints assumed (Helium CAS):
      POST {server}/substances
      GET  {server}/substances/{uri}
    """
    ...
    def _extract_fields(self, details: dict, fallback_smiles: Optional[str] = None, include_full: bool = False, include_wide: bool = False, include_mf_mw: bool = False) -> Dict:
        ...
        out = {
            ...
            "name": _pick_name(rec),  # HTML stripped via _pick_name/_strip_html
        ...
        if include_mf_mw:
            mf = _pick_mf(rec)
            mw = _pick_mw(rec)
            # compute from SMILES if missing
        ...
        # Clean up any lingering HTML fragments across string fields
